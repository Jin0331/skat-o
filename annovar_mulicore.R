...
output <- "0928" ###### input output dir making--- yet
core <- 7
...
temp <- fread(input = "0928_QC_80.recode.vcf", sep = "\t", quote = "\n", skip = 210, header = T, nThread = 14)
...
write.vcf(x = vcf_bind_complete, file = "vcf_QC_0928_80.vcf.gz",APPEND = F);rm(vcf_bind_complete) ## vcf out
system("gzip -d vcf_QC_0928_80.vcf.gz") ## bgzip and tabix
system("bgzip vcf_QC_0928_80.vcf")
system("tabix -p vcf vcf_QC_0928_80.vcf.gz")
system("rm -rf ")



################### skat-o preprocessing #############################
test_fix <- read.table(file = "vcf_bind_0917_fix.txt", sep = "\t", header = T, stringsAsFactors = F)
