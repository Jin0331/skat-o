...
geneset_KFPD<- function(){
  print("Geneset load GO Client geneset!")
  # 92 gene_SLC25A4 remove
  geneset_merge <- fread("https://s3.us-west-2.amazonaws.com/secure.notion-static.com/7b12f84e-e41f-4956-babf-4df080ea1aae/input_gene.txt?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=AKIAT73L2G45O3KS52Y5%2F20210130%2Fus-west-2%2Fs3%2Faws4_request&X-Amz-Date=20210130T152731Z&X-Amz-Expires=86400&X-Amz-Signature=45ded77882058e1dfef3d4313e790b5590afccbedbe08718929f15a7f44a62ef&X-Amz-SignedHeaders=host&response-content-disposition=filename%20%3D%22input_gene.txt%22", 
  ...
  for(gene in index_){ # 2:length(col_name), c(2,3,7,16,17)
    ...
    for(i in 1:length(geneset)){
      variant[[i]] <- subset(fix, subset = ((Gene.knownGene == geneset[i] & Func.knownGene == "exonic")),
                             select = c("CHROM", "POS", "ID", "REF","ALT", "Gene.knownGene","ExonicFunc.knownGene","CADD_PHRED","ID2"))
    }
    ...
    if(nrow(nonsyn_dosage) > 0){
      ...
      nonsyn_result <- mclapply(X = 3:ncol(nonsyn_dosage), FUN = function(col_len){
        ...
        anno_data_nonsyn <- filter(fix, ID2 == temp) %>%
          select(., CHROM, POS, ID2, REF, ALT, Gene.knownGene, AAChange.knownGene, CADD_PHRED, AF, AF_eas, AF_eas_kor) %>% 
    ...
    if(nrow(lof_dosage) > 0){
      ...
      lof_result <- mclapply(X = 3:ncol(lof_dosage), FUN = function(col_len){
        ...
        anno_data_lof <- filter(fix, ID2 == temp) %>%
          select(., CHROM, POS, ID2, REF, ALT, Gene.knownGene, AAChange.knownGene, CADD_PHRED, CADD_PHRED, AF, AF_eas, AF_eas_kor) %>%
          mutate(Clinical_Significance = re_CLNSIG(paste0(CHROM, ":", POS, REF, ALT), clinvar),
                 CLNSIGCONF = re_CLNSIGCONF(paste0(CHROM, ":", POS, REF, ALT), clinvar),
                 CLINVAR_ID = re_CLINVAR_ID(paste0(CHROM, ":", POS, REF, ALT), clinvar),
                 CLNHGVS = re_CLNHGVS(paste0(CHROM, ":", POS, REF, ALT), clinvar)
...
snp_table_sample_count <- function(data_list, type = "Pathogenic", geneset_name, MAF_value = 0.03, CADD_score = 20, data_name,
  ...
  if(type == "Pathogenic"){
    ...
    temp2 <- temp %>% arrange(IID) %>% 
      filter(AF_eas <= MAF_value, Heterozygous != TRUE, CADD_PHRED >= CADD_score) %>%
      snp_table_func1() %>% bind_rows()
    if(nrow(temp2) < 1){
      return(NULL)
    }
  ...
  sample_result <- mclapply(X = sample, FUN = function(iid){
    ...
    # CADD
    {
      for(value in c(0.03, 0.01)){
        CADD_Hetero <- temp %>% 
          filter(AF_eas <= value, CADD_PHRED >= CADD_score,IID == iid, Heterozygous_M == T) %>%
          select(Clinical_Significance) %>%
          group_by_all() %>% count() %>% pull(2) %>% sum() %>% ifelse(length(.) == 0, 0, .)
        
        if(CADD_Hetero != 0){
          Hetero_name <- temp %>% 
            filter(AF_eas <= value, CADD_PHRED >= CADD_score, IID == iid, Heterozygous_M == T) %>% .$ID
          CADD_Hetro_name <- str_c(Hetero_name, collapse = ";");Hetero_name <- NULL
        } else {CADD_Hetro_name <- " "}
        
        CADD_Homo <- temp %>% 
          filter(AF_eas <= value, CADD_PHRED >= CADD_score, IID == iid, Homozygous_M == T) %>%
          select(Clinical_Significance) %>%
          group_by_all() %>% count() %>% pull(2) %>% sum() %>% ifelse(length(.) == 0, 0, .)
        
        if(CADD_Homo != 0){
          Homo_name <- temp %>% 
            filter(AF_eas <= value, CADD_PHRED >= CADD_score, IID == iid, Homozygous_M == T) %>% .$ID
...
snp_table_per_sample_gene <- function(data_list, geneset_name, type, CADD_score = 20){
  ...
  sample_result <- mclapply(X = sample, FUN = function(iid){
    ...
    for(value in c(0.03, 0.01)){
      CADD <- temp %>% 
        filter(AF_eas <= value, CADD_PHRED >= CADD_score, IID == iid, Heterozygous != T) %>% select(Clinical_Significance) %>%        
        group_by_all() %>% count() %>% pull(2) %>% sum() %>% ifelse(length(.) == 0, 0, .)
