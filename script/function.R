...
# SKAT-O FUNCTION ====================
# cov make 
cov_make_not_run <- function(mds_path){
  
  mds <- fread(file = paste0(mds_path, ".mds"), header = T) %>% 
    select(-IID) %>% 
    mutate(IID = FID)
  
  age_control <- fread(file = paste0("/home/dblab/skat-o/SKAT_data/skat_cov/IPDGC_control_phenotype.txt"), select = c(2,7)) %>%
    select(., IID = SUBJECT_ID, AGE = Age)
  age_case <- fread(file = paste0("/home/dblab/skat-o/SKAT_data/skat_cov/IPDGC_case_phenotype.txt"), select = c(2,7)) %>%
    select(., IID = SUBJECT_ID, AGE = Age)
  age <- bind_rows(age_case, age_control) %>% 
    mutate(FID = IID)
  
  # age <- fread(file = "PPMI.age", sep = "\t")
  # age <- age %>% rename(AGE = age)
  
  
  Fmiss <- fread(paste0(mds_path,".imiss")) %>% as_tibble() %>% 
    select(FID, IID, F_MISS)
  
  
  cov <- left_join(x = mds, y = age, by = c("FID","IID")) %>% 
    left_join(x = ., y = Fmiss, by = c("FID","IID")) %>% 
    as_tibble() %>% 
    select(FID, IID, AGE, F_MISS,	C1,	C2,	C3,	C4,	C5,	C6,	C7,	C8,	C9,	C10,	C11,	C12,	C13,	C14,	C15,	C16,	C17,	C18,	C19,	C20)
  
  fwrite(x = cov, file = paste0(mds_path, ".cov"), row.names = F, sep = "\t")
}
...
geneset_load_SKAT <- function(){
  ...
  # 93 gene
  # geneset_merge <- fread(file = "/home/dblab/skat-o/SKAT_data/1 gene sets 190428_exclude_ARX_for SKAT_O.txt", header = T, 
  #                        sep = "\t", stringsAsFactors = F, data.table = F) %>% as_tibble() 
  
  # 92 gene_SLC25A4 remove
  geneset_merge <- fread(file = "/home/dblab/skat-o/SKAT_data/geneset_fix_clinvar/1 gene sets 190428_exclude_ARX_for SKAT_O_SLC25A4_remove.txt", header = T, 
                         sep = "\t", stringsAsFactors = F, data.table = F) %>% as_tibble()
...
fix_load_SKAT <- function(data_name, set_WES = "set2"){
  ...
  if(data_name == "WES_merge" | data_name == "PPMI"){
    test_fix <- fread(file = paste0("/home/dblab/skat-o/SKAT_data/geneset_fix_clinvar/",data_name,"_0823_fix.txt"), sep = "\t", header = T, stringsAsFactors = F, data.table = F) %>% 
    ...
    bim <- fread(file = paste0("/home/dblab/skat-o/SKAT_data/geneset_fix_clinvar/", data_name, "_0821.bim")) %>% 
      as_tibble() %>% select(ID2 = V2)
    
    test_fix <- bind_cols(test_fix, bim)
    
    bim2 <- fread(file = paste0("/home/dblab/skat-o/SKAT_data/", set_WES, "/", data_name, "_0821.bim")) %>% 
      as_tibble() %>% select(ID2 = V2)
    
    test_fix <- left_join(x = bim2, y = test_fix, by = "ID2")
    
    freq <- fread(file = paste0("/home/dblab/skat-o/SKAT_data/",set_WES,"/",data_name,"_0821.frq"), header = T) %>%
      rename(ID2 = SNP) %>% 
      as_tibble()
    ...
    test_fix <- fread(file = paste0("/home/dblab/skat-o/SKAT_data/geneset_fix_clinvar/",data_name, "_fix.txt"), sep = "\t", header = T, stringsAsFactors = F, data.table = F) %>% 
      as_tibble()
    test_fix$CHROM <- gsub(x = test_fix$CHROM, pattern = "(X)", replacement = "23")
    test_fix$CHROM <- gsub(x = test_fix$CHROM, pattern = "(Y)", replacement = "24")
    ...
    freq <- fread(file = paste0("/home/dblab/skat-o/SKAT_data/",set_WES,"/", data_name, "_0821.frq")) %>%
      rename(ID2 = SNP) %>% 
      as_tibble()
    test_fix <- left_join(x = test_fix, y = freq, by = "ID2")
    
    test_fix$CADD13_PHRED[test_fix$CADD13_PHRED == "."] <- 0
    test_fix <- test_fix %>% mutate(CADD13_PHRED = as.numeric(CADD13_PHRED))
...
geneset_setid <- function(geneset_merge, col_name, gene_count, test_fix, data_name, index_, CADD_score, geneset_type = "ALL"){
...
gene_setid <- function(index = "default_all", geneset, test_fix, data_name, CADD_score){
  ...
  if(is.character(index)){ # if 1, all gene
    ...
    temp %>% bind_rows() %>% select(Gene.knownGene, ID2) %>% 
      write_delim(path = paste0(data_name, "_skat_gene.SetID"), delim = "\t", col_names = F, append = T)
...
make_obj <- function(data_name, flag = "geneset", re = 0, add_name, set_WES = "set2"){
  if(data_name == "WES_merge" | data_name == "PPMI" | data_name == "NeuroX"){
      Generate_SSD_SetID(File.Bed = paste0("/home/dblab/skat-o/SKAT_data/", set_WES, 
                                           "/",data_name, "_0821.bed"),
                         File.Bim = paste0("/home/dblab/skat-o/SKAT_data/", set_WES, 
                                           "/",data_name, "_0821.bim"), 
                         File.Fam = paste0("/home/dblab/skat-o/SKAT_data/", set_WES, 
                                           "/",data_name, "_0821.fam"),
    ...
    if(data_name == "WES_merge" | data_name == "PPMI"){
        FAM <- Read_Plink_FAM_Cov(Filename = paste0("/home/dblab/skat-o/SKAT_data/", set_WES, "/",data_name, "_0821.fam"),
                                  File_Cov = paste0("/home/dblab/skat-o/SKAT_data/", set_WES, "/",data_name, "_0821.cov"), Is.binary = F)
    } else if(data_name == "NeuroX"){
        FAM <- Read_Plink_FAM_Cov(Filename = paste0("/home/dblab/skat-o/SKAT_data/", set_WES, "/",data_name, "_0821.fam"),
                                  File_Cov = paste0("/home/dblab/skat-o/SKAT_data/", set_WES, "/",data_name, "_0821.cov"), Is.binary = T)
    }
...
run_skat_all_cov <- function(data_name, flag = "geneset", re = 0, add_name, set_WES = "set2"){
  ...
  if(flag == "geneset"){
    ...
    file.remove(list.files()[!str_detect(list.files(), pattern = "SKATO|MAC|SetID")])
    ...
    if(data_name == "WES_merge" | data_name == "PPMI" | data_name == "NeuroX"){
      Generate_SSD_SetID(File.Bed = paste0("/home/dblab/skat-o/SKAT_data/", set_WES, "/",data_name, "_0821.bed"),
                         File.Bim = paste0("/home/dblab/skat-o/SKAT_data/", set_WES, "/",data_name, "_0821.bim"), 
                         File.Fam = paste0("/home/dblab/skat-o/SKAT_data/", set_WES, "/",data_name, "_0821.fam"),
...
MAC_calculation <- function(data_name, add_name, set_IPDGC){
  ...
  if(data_name == "IPDGC"){
    Generate_SSD_SetID(File.Bed = paste0("/home/dblab/skat-o/SKAT_data/SKAT_set_data/", set_IPDGC, 
                                         "/",data_name,"_", set_IPDGC, ".bed"),
                       File.Bim = paste0("/home/dblab/skat-o/SKAT_data/SKAT_set_data/", set_IPDGC, 
                                         "/",data_name,"_", set_IPDGC, ".bim"), 
                       File.Fam = paste0("/home/dblab/skat-o/SKAT_data/SKAT_set_data/", set_IPDGC, 
                                         "/",data_name,"_", set_IPDGC, ".fam"),
                       File.SetID = paste0(data_name,"_skat_gene.SetID"), 
                       File.SSD = paste0(data_name,"_gene.SSD"), 
                       File.Info = paste0(data_name,"_gene.INFO"))
    
    FAM <- Read_Plink_FAM_Cov(Filename = paste0("/home/dblab/skat-o/SKAT_data/SKAT_set_data/", set_IPDGC, 
                                                "/",data_name,"_", set_IPDGC, ".fam"),
                              File_Cov = paste0("/home/dblab/skat-o/SKAT_data/SKAT_set_data/", set_IPDGC, 
                                                "/",data_name,"_", set_IPDGC, ".cov"), Is.binary = FALSE )
  } else {
    
    Generate_SSD_SetID(File.Bed = paste0("/home/dblab/skat-o/SKAT_data/",data_name,".bed"),
                       File.Bim = paste0("/home/dblab/skat-o/SKAT_data/",data_name,".bim"), 
                       File.Fam = paste0("/home/dblab/skat-o/SKAT_data/",data_name,".fam"),
                       File.SetID = paste0(data_name,"_skat_gene.SetID"), 
                       File.SSD = paste0(data_name,"_gene.SSD"), 
                       File.Info = paste0(data_name,"_gene.INFO"))
    
    FAM <- Read_Plink_FAM_Cov(Filename = paste0("/home/dblab/skat-o/SKAT_data/",data_name,".fam"),
                              File_Cov = paste0("/home/dblab/skat-o/SKAT_data/",data_name,".cov"), Is.binary = FALSE )
  }
  SSD.INFO <- Open_SSD(File.SSD = paste0(data_name,"_gene.SSD"), File.Info = paste0(data_name,"_gene.INFO"))
...
# preprocessing
fix_load <- function(data_name, type = "QC", set_WES = "set2"){
  print(paste0(data_name, " fix load!"))
  
  
  if(type == "ROW"){
    test_fix <- fread(file = paste0("/home/dblab/skat-o/SKAT_data/geneset_fix_clinvar/",data_name, "_row_0823_fix.txt"), sep = "\t", header = T, stringsAsFactors = F, data.table = F) %>% 
      as_tibble() 
    test_fix$CHROM <- gsub(x = test_fix$CHROM, pattern = "(X)", replacement = "23")
    test_fix$CHROM <- gsub(x = test_fix$CHROM, pattern = "(Y)", replacement = "24")
  ...
  } else if(data_name == "NeuroX" & type != "ROW") { # NeuroX
    ...
    bim <- fread(file = paste0("/home/dblab/skat-o/SKAT_data/",set_WES,"/", data_name, "_0821.bim")) %>% 
      as_tibble() %>% select(ID2 = V2)
...
} # data_name, "WES", "PPMI", "NeuroX"

clinvar_load <- function(){
  # clinvar preprocessing
  clinvar <- fread(file = "/home/dblab/skat-o/SKAT_data/geneset_fix_clinvar/clinvar_20190506.vcf", header = T) %>%
...
geneset_extract <- function(geneset_merge, col_name, test_fix, data_name, index_){
  ...
  for(gene in index_){ # 2:length(col_name), c(2,3,7,16,17)
    ...
    for(i in 1:length(geneset)){
      variant[[i]] <- subset(test_fix, subset = ((Gene.knownGene == geneset[i] & Func.knownGene == "exonic")),
                             select = c("CHROM", "POS", "ID", "REF","ALT", "Gene.knownGene","ExonicFunc.knownGene","CADD13_PHRED","ID2"))
    }
    variant <- bind_rows(variant)
...
snp_table <- function(data_name = "WES_merge", index_, clinvar, set_type = 2){
  core <- detectCores() - 1
  geneset_result <- list()
  ...
  for(index in index_){
    # system(glue("plink --bfile /home/dblab/skat-o/SKAT_data/{data_name} --extract {data_name}_{geneset}_non-syn.txt --recode A --out test_dosage_nonsyn",
    #             data_name = data_name, geneset = geneset[[2]][index]), ignore.stdout = T)
    # system(glue("plink --bfile /home/dblab/skat-o/SKAT_data/{data_name} --extract {data_name}_{geneset}_lof.txt --recode A --out test_dosage_lof",
    #             data_name = data_name, geneset = geneset[[2]][index]), ignore.stdout = T)
    
    system(glue("plink --bfile /home/dblab/skat-o/SKAT_data/set{type}/{data_name}_0821 --extract {data_name}_{geneset}_non-syn.txt --recode A --out test_dosage_nonsyn",
                data_name = data_name, geneset = geneset[[2]][index], type = set_type), ignore.stdout = T)
    system(glue("plink --bfile /home/dblab/skat-o/SKAT_data/set{type}/{data_name}_0821 --extract {data_name}_{geneset}_lof.txt --recode A --out test_dosage_lof",
                data_name = data_name, geneset = geneset[[2]][index], type = set_type), ignore.stdout = T)
    ...
    nonsyn_result <- mclapply(X = 4:ncol(nonsyn_dosage), FUN = function(col_len){
      ...
      if(!str_detect(string = colnames(nonsyn_dosage)[col_len], pattern = "^rs")){
        temp <- paste(str_split(colnames(nonsyn_dosage)[col_len], pattern = "_")[[1]][1],
                      str_split(colnames(nonsyn_dosage)[col_len], pattern = "_")[[1]][2], sep = "_")
      } else {
        temp <- str_split(colnames(nonsyn_dosage)[col_len], pattern = "_")[[1]][1]
      }
      
      anno_data_nonsyn <- filter(fix, ID2 == temp) %>%
        select(., CHROM, POS, ID2, REF, ALT, Gene.knownGene, AAChange.knownGene, CADD13_PHRED, MAF) %>% 
    ...
    lof_result <- mclapply(X = 4:ncol(lof_dosage), FUN = function(col_len){
      ...
      if(!str_detect(string = colnames(lof_dosage)[col_len], pattern = "^rs")){
        temp <- paste(str_split(colnames(lof_dosage)[col_len], pattern = "_")[[1]][1],
                      str_split(colnames(lof_dosage)[col_len], pattern = "_")[[1]][2], sep = "_")
      } else {
...
snp_table_ALL <- function(data_name = "WES_merge", index_, clinvar, set_type = 2){
  core <- detectCores() - 1
  geneset_result <- list()
  geneset <- geneset_load_SKAT()
  fix <- fix_load(data_name, type = "QC")
...
table3_ver_1 <- function(data_list, type = "Pathogenic", geneset_name, MAF_value = 0.03, CADD_score = 20, set = 2, data_name = "WES_merge"){
  ...
  # if
  if(data_name == "WES_merge"){
    if(result[1,1] == 0)
      result[1,1] <- 430 - result[,1] %>% sum
    if(result[1,2] == 0)
      result[1,2] <- 334 - result[,2] %>% sum  
  }else if(data_name == "PPMI"){
    if(result[1,1] == 0)
      result[1,1] <- 347 - result[,1] %>% sum
    if(result[1,2] == 0)
      result[1,2] <- 150 - result[,2] %>% sum
  }
...
table3_ver_3_sample_gene <- function(data_list, geneset_name, type, CADD_score = 20){
  ...
  sample_result <- mclapply(X = sample, FUN = function(iid){
    ...
    for(value in c(0.03, 0.01)){
      ...
      if(CADD != 0){
        CADD_name_He <- temp %>% 
          filter(MAF <= value, CADD13_PHRED >= CADD_score, IID == iid, Heterozygous_M == T) %>% .$ID2 %>% as_tibble()
        CADD_ID_He <- temp %>% 
          filter(MAF <= value, CADD13_PHRED >= CADD_score, IID == iid, Heterozygous_M == T) %>% 
        ...
        CADD_name_Ho <- temp %>% 
          filter(MAF <= value, CADD13_PHRED >= CADD_score, IID == iid, Homozygous_M == T) %>% .$ID2 %>% as_tibble()
      ...
      if(value == 0.03){
        ...
        CADD_MAF003_CADD_HOMO <- CADD_ID_Ho_verfy
        
      } else {
        CADD_MAF001_ID <- CADD_ID %>% 
          name_change(ID_DF = ., type = "CADD_MAF001")
        CADD_MAF001_CADD_HOMO <- CADD_ID_Ho_verfy
      }
    ...
    snp_table <- switch(type, "Pathogenic" = Pathogenic_ID,
                        ...
                        "ALL" = bind_cols(Pathogenic_ID, CADD_MAF003_CADD_HOMO,CADD_MAF003_ID) %>% as_tibble() # only CADD 003
    )
...
geneset_setid_synonymous_not_run <- function(geneset_merge, col_name, test_fix, data_name, index_, CADD_score){
  print("geneset SetID making!!")
  pb <- progress_bar$new(total = length(index_), clear = F)
  pb$tick(0)
  Sys.sleep(0.01)
