...
# tool path 
Sys.setenv(PATH = "/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/home/jinoo/tool/:/home/jinoo/annovar/:/home/lee/epacts_0913/bin/")
...
## unknown snp QC(e.g. minor | major 0)
{
  ...
  write.table(x = bim_zero_snp, file = "remove_snp_file_0328.txt", sep = "\t", quote = F, row.names = F, col.names = F)
  
  system("mkdir data");setwd("data/")
  system("plink --bfile ../dbGaP_NeuroX_filter --exclude ../remove_snp_file_0328.txt --make-bed --out skatQC")
  system("./update_build.sh skatQC ../NeuroX_15036164_A-b37.Source.strand skatQC_strand") ### strand update
...
## sample QC plink
{
  ...
  system("plink --bfile /home/jinoo/skat-o/NeuroX/data/skatQC_strand --het --out skatQC_het") ### heterogygosity
  system("plink --bfile /home/jinoo/skat-o/NeuroX/data/skatQC_strand --indep-pairwise 50 5 0.2 --out skat_pruning")
  system("plink --bfile /home/jinoo/skat-o/NeuroX/data/skatQC_strand --genome full --out skatQC_IBD") ### MDS
  system("plink --bfile /home/jinoo/skat-o/NeuroX/data/skatQC_strand --read-genome skatQC_IBD.genome --extract skat_pruning.prune.in --mds-plot 20 --cluster --out skatQC_MDS")
  system("plink --bfile /home/jinoo/skat-o/NeuroX/data/skatQC_strand --missing --out skatQC_missing") ### individual missing
  system("plink --bfile /home/jinoo/skat-o/NeuroX/data/skatQC_strand --check-sex --out skatQC_sex")
}
...
## QC and vcf
{
  ...
  system("plink --bfile ../data/skatQC_strand --geno 0.01 --hwe 1e-6 --make-bed --out skatQC_strand_snp --noweb")
  system("plink --bfile skatQC_strand_snp --recode vcf-iid --allow-no-sex --out ../NeuroX_filter_qc")
  setwd("../")
  ...
  system("vcftools --gzvcf NeuroX_filter_qc.vcf.gz --remove plink/plink_ind.txt --recode --out NeuroX_vcftool_qc.flt")
}
...
### anotation
{
  ...
  temp <- fread(input = "NeuroX_vcftool_qc.flt.recode.vcf", sep = "\t", quote = "\n", skip = 30, header = T) %>%
    as_tibble()### if qc, skip = 3 and select remove
  
  vcf_list <- list()
  vcf_list[1] <- list(temp[1:round(nrow(temp)/core),])
  for(i in 2:(core)){
    if(i == core){
      vcf_list[i] <- list(temp[((nrow(vcf_list[[1]])*(i-1))+1):(nrow(temp)),])
      break;
    }
    vcf_list[i] <- list(temp[((nrow(vcf_list[[1]])*(i-1))+1):(nrow(vcf_list[[1]])*i),])
  }
  rm(temp);gc()
...
## for annotation, vcf out
  ...
  system.time(
    foreach(i=1:core) %dopar% {
      system(glue("table_annovar.pl {path[i]} /home/jinoo/annovar/humandb/ -buildver hg19 -out {path[i]} -remove -protocol knownGene,cadd13gt10 -operation g,f -nastring . -vcfinput"))
    }
  );gc() #
  ...
  rm(mul_vcf)
  fwrite(x = mul_anno_fix, file = "../NeuroX_annotation_fix_source_strand.txt", quote = F, sep = "\t", row.names = F, col.names = T, eol = "\n")
