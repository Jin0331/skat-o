...

{
  ...
  #   
  # }
  # SKAT_gene_MAC <- function(data_name, table){
  #   FAM <- fread(file = paste0("/home/jinoo/skat-o/SKAT_data/", data_name,".fam")) %>% as_tibble()
  #   FAM <- FAM %>% select(V1, V2, V6) %>% rename(FID = V1, IID = V2, PHENOTYPE = V6) %>% 
  #     mutate(PHENOTYPE = ifelse(PHENOTYPE == 1, "control", "case")) %>% 
  #     write_delim(path = "cluster.txt", delim = "\t", col_names = F)
  #   system(glue("plink --bfile /home/jinoo/skat-o/SKAT_data/{data_name} --freq --within cluster.txt", data_name = data_name), ignore.stdout = T)
  #   case_control_mac <- fread("plink.frq.strat") %>% 
  #     select(-A1,-A2, -MAF, -NCHROBS) %>% as_tibble() %>% 
  #     spread(key = CLST, value = MAC)
  #   
  #   system("rm -rf cluster.txt")
  #   system("rm -rf plink.frq.strat")
  #   system("rm -rf plink.log")
  #   system("rm -rf plink.hh")
  #   
  #   gene_wes_table <- table[[1]] %>% filter(Func == "Nonsynonymous", CADD13_PHRED >= 20)
  #   
  #   result <- mclapply(X = unique(gene_wes_table$Gene.knownGene), FUN = function(gene){
  #     
  #     DF <- gene_wes_table %>% filter(Gene.knownGene == gene) %>% pull(3)
  #     gene_case_control_mac <- case_control_mac %>% filter(SNP %in% DF)
  #     
  #     control_mac <- gene_case_control_mac %>% pull(control) %>% sum
  #     case_mac <- gene_case_control_mac %>% pull(case) %>% sum
  #     variant <- length(DF)
  #     
  #     tibble(GENE = gene, Variant = variant, MAC = (control_mac + case_mac), Control_MAC = control_mac, Case_MAC = case_mac) %>% 
  #       return()
  #   }, mc.cores = 3)
  #   
  #   return(result)
