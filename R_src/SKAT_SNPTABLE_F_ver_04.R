...
fix_load <- function(data_name, type = "QC", set_WES = "set2"){
  ...
  if(type == "ROW"){
    test_fix <- fread(file = paste0("/home/jinoo/skat-o/SKAT_data/geneset_fix_clinvar/",data_name, "_row_0823_fix.txt"), sep = "\t", header = T, stringsAsFactors = F, data.table = F) %>% 
      as_tibble() 
    test_fix$CHROM <- gsub(x = test_fix$CHROM, pattern = "(X)", replacement = "23")
    test_fix$CHROM <- gsub(x = test_fix$CHROM, pattern = "(Y)", replacement = "24")
...
snp_table <- function(data_name = "WES_merge", index_, clinvar, set_type = 2){
  ...
  for(index in index_){
    ...
    system(glue("plink --bfile /home/jinoo/skat-o/SKAT_data/set{type}/{data_name}_0821 --extract {data_name}_{geneset}_non-syn.txt --recode A --out test_dosage_nonsyn",
                data_name = data_name, geneset = geneset[[2]][index], type = set_type), ignore.stdout = T)
    system(glue("plink --bfile /home/jinoo/skat-o/SKAT_data/set{type}/{data_name}_0821 --extract {data_name}_{geneset}_lof.txt --recode A --out test_dosage_lof",
                data_name = data_name, geneset = geneset[[2]][index], type = set_type), ignore.stdout = T)
