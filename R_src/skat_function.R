...
MAC_calculation <- function(data_name, add_name){
  ...
  MAC_result <- mclapply(X = 1:nrow(SSD.INFO$SetInfo), FUN = function(SET_index){
      ...
      # # sum
      # case_MAF <- Z_case %>% apply(X = ., MARGIN = 1, function(temp) sum(temp, na.rm = T)) %>% mean(na.rm = T)/2
      # control_MAF <- Z_control %>% apply(X = ., MARGIN = 1, function(temp) sum(temp, na.rm = T)) %>% mean(na.rm = T)/2
      
      # # average
      # case_MAF <- (colMeans(Z_case, na.rm = T)/2) %>% mean(na.rm = T)
      # control_MAF <- (colMeans(Z_control, na.rm = T)/2) %>% mean(.,na.rm = T)
      # #
  ...
  MAC_result %>% bind_rows() %>% 
    fwrite(file = paste0(data_name,"_MAC_gene_",add_name,".txt"), col.names = T, row.names = F, sep = "\t")
  Close_SSD()
}
...
{
  # run_skat_all_common_rare_cov_not_run <- function(data_name, flag = "geneset", re = 0){
  #   print("SKAT run")
  #   if(flag == "geneset"){
  #     
  #     Generate_SSD_SetID(File.Bed = paste0("/home/jinoo/skat-o/SKAT_data/",data_name,".bed"),
  ...
  # }
  # ### cov making
  # cov_make_not_run <- function(name = "IPDGC"){
  ...
  #   ###
  # 
  #   FAM <- Read_Plink_FAM(Filename = paste0("/home/jinoo/skat-o/SKAT_data/",name, ".fam"))
  # 
  #   mds <- fread(file = "/home/jinoo/skat-o/SKAT_data/skat_cov/", name,"_MDS.mds", header = T) %>% select(., -SOL)
  #   age_control <- fread(file = paste0("/home/jinoo/skat-o/SKAT_data/skat_cov/", name ,"_control_phenotype.txt"), select = c(2,7)) %>%
  #     select(., IID = SUBJECT_ID, AGE = Age)
  #   age_case <- fread(file = paste0("/home/jinoo/skat-o/SKAT_data/skat_cov/", name,"_case_phenotype.txt"), select = c(2,7)) %>%
  #     select(., IID = SUBJECT_ID, AGE = Age)
  #   age <- bind_rows(age_case, age_control)
  #   
  #   system("plink --bfile ../IPDGC --missing --out IPDGC_F_MISS")
  #   Fmiss <- fread(file = "IPDGC_F_MISS.imiss") %>% select(IID, F_MISS)
  #   
  #   cov <- left_join(x = cov, y = Fmiss, by = "IID")
  ...
  # 
  #   fwrite(x = select(cov, -PID, -MID, -Sex, -Phenotype), file = "IPDGC.cov", row.names = F, sep = "\t")
  #   # fwrite(x = select(NeuroX_cov, -PID, -MID, -Sex, -Phenotype), file = "NeuroX.cov", row.names = F, sep = "\t")
  # 
  #   FAM <- Read_Plink_FAM_Cov(Filename = "/home/jinoo/skat-o/SKAT_data/IPDGC.fam",
  #                             File_Cov = "/home/jinoo/skat-o/SKAT_data/IPDGC.cov",Is.binary = F)
  # }
