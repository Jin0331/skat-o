# library path
{
  ...
  library(tidyverse)
# tool path 
  Sys.setenv(PATH = "/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/home/jinoo/tool/:/home/jinoo/annovar/:")
}
...
## sample QC
{
  ...
  #Annotation
  {
    ...
    core <- detectCores() -1
    temp_tidy <- fread(input = "../IPDGC_row_merge.vcf", sep = "\t", quote = "\n", skip = 210, header = T) %>% as_tibble()
    ...
    system.time(
      foreach(i=1:core) %dopar% {
        system(glue("table_annovar.pl {path[i]} /home/jinoo/annovar/humandb/ -buildver hg19 -out {path[i]} -remove -protocol knownGene -operation g -nastring . -vcfinput"))
      }
    );gc() #
    ...
    rm(mul_vcf)
    fwrite(x = mul_anno_fix, file = "../NeuroX_row_fix.txt", quote = F, sep = "\t", row.names = F, col.names = T, eol = "\n")
